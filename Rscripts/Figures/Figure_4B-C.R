...
if (!file.exists(list.files(de_ase_dir,full.names = TRUE)[1])) {stop(" No SpliceWiz DESeq2 files exist.\n\tRun SpliceWiz.R script to generate.", call. = FALSE)} # check file exists
...
df_early = read.csv(file.path(de_ase_dir, "DE.WT_Early.vs.WT_Latent.csv"))
df_late = read.csv(file.path(de_ase_dir, "DE.WT_Late.vs.WT_Latent.csv"))

# Load differential expressed genes
deg_early = read.csv(file.path(deg_dir, "ERCCnorm_DE.WT_Early.vs.WT_Latent.csv"))
deg_late = read.csv(file.path(deg_dir, "ERCCnorm_DE.WT_Late.vs.WT_Latent.csv"))
...
df_early$Condition = "WT_Early"
df_late$Condition = "WT_Late"
df = rbind(df_early, df_late)

# Volcano plot function
ggvaplot_fun = function(tmp_df,ASE, xlim = 10, ylim = 60) {
  ...
  # Factor by fraction
  tmp_df$Condition = factor(tmp_df$Condition, levels = c("WT_Early","WT_Late"))
  ...
  ggplot(tmp_df, aes(x = log2FoldChange, y = -log10(padj), fill = Sig)) +
    ...
    scale_y_continuous(limits = c(-2,ylim), breaks = seq(0,ylim,10), expand = c(0,0.75), oob = squish) +
    scale_x_continuous(limits = c(-xlim,xlim), breaks = seq(-xlim,xlim,5), expand = c(0,0.15), oob = squish) +
...
write.csv(ir_out_table,file.path(out_dir,"Figure_4B.Table_events.csv"), row.names=FALSE)
write.csv(se_out_table,file.path(out_dir,"Figure_4C.Table_events.csv"), row.names=FALSE)
...
aseGeneTable_fun = function(tmp_df, ASE) {
  ...
  tmp_df1 = tmp_df[tmp_df$Condition == "WT_Early" & tmp_df$Sig == "Down",]
  tmp_df2 = tmp_df[tmp_df$Condition == "WT_Early" & tmp_df$Sig == "Up",]
  tmp_df3 = tmp_df[tmp_df$Condition == "WT_Late" & tmp_df$Sig == "Down",]
  tmp_df4 = tmp_df[tmp_df$Condition == "WT_Late" & tmp_df$Sig == "Up",]
...
write.csv(ir_gene_out_table,file.path(out_dir,"Figure_4B.Table_genes.csv"), row.names=FALSE)
write.csv(se_gene_out_table,file.path(out_dir,"Figure_4C.Table_genes.csv"), row.names=FALSE)
...
# Intron Retention
ggvaplot_fun(df, "IR") + theme_white
ggsave(file.path(out_dir,"Figure_4B.png"), dpi = 300, width = 4.25, height = 7.52, units = "in")

# Exon Skipping
ggvaplot_fun(df, "SE") + theme_white
ggsave(file.path(out_dir,"Figure_4C.png"), dpi = 300, width = 4.25, height = 7.52, units = "in")

# Background
background_plot = ggvaplot_fun(df, "IR")
background_plot$layers <- NULL
background_plot
ggsave(file.path(out_dir,"Figure_4.Background.svg"), dpi = 300, width = 4.25, height = 7.52, units = "in")
