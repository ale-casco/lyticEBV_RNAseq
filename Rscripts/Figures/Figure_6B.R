...
# Output directory
out_dir=file.path(work_dir, "Figures/Figure_6B")
dir.create(out_dir, showWarnings=FALSE, recursive=TRUE)

# location of files
base=file.path(work_dir, "Data/Flow_Data/Figure_6B")
...
# set debris gate
g.debris <- polygonGate(filterId = "Debris","FSC.A"=c(3e5,7.5e5,7.5e5,3e5,1.75e5,2e5),"SSC.A"=c(0.1e5,0.5e5,3.5e5,2.5e5,0.75e5,0.25e5))
...
# set singlet gate
g.singlets <- polygonGate(filterId = "Singlets","FSC.A"=c(1.7e5,8e5,8e5,6e5,1.7e5),"FSC.H"=c(0.7e5,3.3e5,4.6e5,4.4e5,3e5))
#ggcyto(gs,aes(x=FSC.A,y=FSC.H),subset="Debris")+geom_hex(bins = 200)+geom_gate(g.singlets) +
#  ggcyto_par_set(limits = list(x = c(1.7e5,8e5), y = c(0.5e5, 4.5e5))) + geom_stats()
gs_pop_add(gs,g.singlets,parent="Debris")
recompute(gs)

# set live gate
g.live <- polygonGate(filterId = "Live","FSC.A"=c(1.7e5,7.55e5,7.55e5,1.7e5),"DAPI"=c(-1,-1,1.35,1.35))
#ggcyto(gs,aes(x=FSC.A,y=DAPI),subset="Singlets")+geom_hex(bins = 200)+geom_gate(g.live) +
#  ggcyto_par_set(limits = list(x = c(1.7e5,7.5e5), y = c(-1, 4))) + geom_stats()
gs_pop_add(gs,g.live,parent="Singlets")
recompute(gs)

# Set GFP-/RFP- (latent), GFP+/RFP- (early), and GFP+/RFP+ (late) gates
g.latent <- polygonGate(filterId = "Latent", "RFP" = c(-0.4,1.5,1.85,1.85,1.6,-0.4), "GFP" = c(-0.4,-0.4,1.4,1.83,1.83,0.83))
g.early <- polygonGate(filterId = "Early", "RFP" = c(-0.4,1.6,1.6,-0.4), "GFP" = c(1.03,2.03,4.3,4.3))
g.late <- polygonGate(filterId = "Late", "RFP" = c(1.8,3.9,3.9,1.8), "GFP" = c(2.03,2.03,4.3,4.3))
#ggcyto(gs, aes(x = "RFP", y = "GFP"),  subset = "Live") + geom_hex(bins=200) + 
#  geom_gate(g.latent) + geom_gate(g.early) + geom_gate(g.late) +
#  ggcyto_par_set(limits = list(x = c(-0.5,4), y = c(-0.5,4))) + geom_stats()
...
# Change names
pData(gs)$name = c("Stimulated +GCV", "Stimulated", " Unstimulated")
...
ggcyto(gs, aes(x = "RFP", y = "GFP"),  subset = "Live") + geom_hex(bins=100) + 
  ...
  ggcyto_par_set(limits = list(x = c(-0.5,4), y = c(-0.5,4))) +
  #axis_y_inverse_trans() + axis_x_inverse_trans() +
  facet_wrap(~name, ncol = 4) +
  labs(x = "mScarlet-I",y = "mGreenLantern") +
  theme_white

ggsave(file.path(out_dir,"Figure_6B.png"), dpi = 300, width = 7.5, height = 3.3, units = "in")
...
ggsave(file.path(out_dir,"Figure_6B.Background.svg"), dpi = 300, width = 7.5, height = 3.3, units = "in")
