...
# location of files
base= file.path(work_dir,"Data_Raji/metrics/host")

# Sample information file
sampleinfo_dir= file.path(work_dir,"Data_Raji/refs/sampleinfo.txt")
...
df$Fraction = factor(df$Fraction, levels = c("Latent","Early"))

# Factor data
df_plot = df
df_plot$Fraction =  factor(df_plot$Fraction, levels = c("Latent","Early"))
...
df_out$Condition = factor(df_out$Condition, levels = c("Latent","Early"))
...
ggsave(file.path(out_dir,"Supplementary_Figure_6D.svg"), dpi = 300, width = 5.21, height = 5.21, units = "in")
