...
latent_list = list()
for (i in which(grepl("dBGLF5_Latent_c", colnames(read_ins)))) {
  tmp = read_ins[,c(1,i)]
...
early_list = list()
for (i in which(grepl("dBGLF5_Early_c", colnames(read_ins)))) {
  tmp = read_ins[,c(1,i)]
  tmp = tmp$ENSEMBL[tmp[,2] == "Read-In"]
...
late_list = list()
for (i in which(grepl("dBGLF5_Late_c", colnames(read_ins)))) {
  tmp = read_ins[,c(1,i)]
  tmp = tmp$ENSEMBL[tmp[,2] == "Read-In"]
...
# Export as SVG
ggsave(file.path(out_dir, "Supplementary_Figure_8G.svg"), width = 5, height = 5)
