...
# Comparisons to keep
files = files[grepl("WT_Early.vs.WT_Latent|WT_Late.vs.WT_Latent|dBGLF5_Early.vs.dBGLF5_Latent|dBGLF5_Late.vs.dBGLF5_Latent",files)]
...
# Keep dBGLF5 condition
deg_table = deg_table[grepl("dBGLF5",rownames(deg_table)),]

# Export DEG table
write.csv(deg_table,file.path(out_dir,"Figure_7A.Table.csv"))
...
ggsave(file.path(out_dir,"Figure_7A.Early.png"), dpi = 300, width = 5.21, height = 5.21, units = "in")

# dBGLF5 Late
plot_list[[2]] + theme_white
ggsave(file.path(out_dir,"Figure_7A.Late.png"), dpi = 300, width = 5.21, height = 5.21, units = "in")
dev.off()
...
background_plot
ggsave(file.path(out_dir,"Figure_7A.Background.svg"), dpi = 300, width = 5.21, height = 5.21, units = "in")
...
# Factor contrast
df$Contrast = factor(df$Contrast, levels = c("WT_Early.vs.WT_Latent","dBGLF5_Early.vs.dBGLF5_Latent","WT_Late.vs.WT_Latent","dBGLF5_Late.vs.dBGLF5_Latent"))

# Due to large sample, we will assume normality.
plot_export = ggbetweenstats(data = df, x = Contrast, y = log2FoldChange, var.equal = FALSE, p.adjust.method = "fdr", point.args = list(alpha = 0)) +
  scale_y_continuous(limits = c(-8,12.5), breaks = seq(-8,8,2), expand = c(0,0.15), oob = squish)

# Welch’s t-test
library(rstatix)
t_test.result = df %>%
  pairwise_t_test(log2FoldChange ~ Contrast,
                  p.adjust.method = "fdr",
                  pool.sd = FALSE) # Welch's t-test
t_test.result

# Export violin plot as SVG
plot_export$layers[[1]] <- NULL # removes hidden points to decrease file size
plot_export
ggsave(file.path(out_dir,"Figure_7B.svg"), dpi = 10, width = 3.82, height = 4.91, units = "in")
