...
if (!file.exists(iurpred2a_path)) {
  outFile <- file(iurpred2a_path, "w")
  for (i in file.path(dirname(iurpred2a_path),list.files(dirname(iurpred2a_path),pattern = "isoformSwitchAnalyzeR_isoform_AA_subset*"))) {
    x <- readLines(i)
