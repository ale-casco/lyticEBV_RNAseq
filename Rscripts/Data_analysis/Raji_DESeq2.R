...
out_dir=file.path(work_dir, "Data_Raji")

# Set directory containing RSEM counts files
counts_dir=file.path(work_dir, "Data_Raji/RSEM")

# Set directory for sampleinfo matrix file
sampleinfo_dir=file.path(work_dir, "Data_Raji/refs/sampleinfo.txt") # 3-column file: id, condition, replicate
...
contrasts_list=list(c("Early","Latent"))

## Differential Gene Expression Analysis

# Create working directory
out_dir=file.path(work_dir, "Data_Raji/DESeq2")
...
for (i in 1:length(contrasts_list)) {
  ...
  #write.table(sampleinfo,file.path(out_dir, "counts",'sampleinfo_IGV.txt'), row.names = F, quote = F, sep = '\t')
  ...
  #colnames(res)<-c(paste0("Log2fc_",colnames(contrasts)[1]),paste0("P.value_",colnames(contrasts)[1]),paste0("Adj.p.value_",colnames(contrasts)[i]))
